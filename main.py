...
from kivy.config import Config
Config.set('kivy','window_icon','images/icon.jpg')
...
class MainWidget(RelativeLayout):
    ...
    menu_title = StringProperty("CAR ON TRACK")
    ...
    def is_desktop(self):
        if platform.system() in ('Linux', 'Windows', 'macosx'):
            return True
        return False
    ...
    def check_char_collision(self):
        for i in range(0, len(self.tiles_coordinates)):
            ...
            if ti_y > self.current_y_loop + 1:
                return False
            if self.check_char_collision_with_tile(ti_x, ti_y):
                return True
        return False

    def check_char_collision_with_tile(self, ti_x, ti_y):
        xmin, ymin = self.get_tile_coordinates(ti_x, ti_y)
        xmax, ymax = self.get_tile_coordinates(ti_x + 1, ti_y + 1)
        for i in range(0, 3):
            px, py = self.char_coordinates[i]
    ...
    def update(self, dt):
        ...
        if not self.check_char_collision() and not self.state_game_over:
